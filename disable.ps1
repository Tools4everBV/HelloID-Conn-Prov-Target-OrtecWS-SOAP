##################################################
# HelloID-Conn-Prov-Target-OrtecWS-SOAP-Disable
# PowerShell V2
##################################################
...
try {
    ...
    switch ($lifecycleProcess) {
        'DisableAccount' {
            Write-Information "Disabling Ortec-WS account with accountReference: [$($actionContext.References.Account)]"
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                ...
                Write-Information "[DryRun] Disable Ortec-WS account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    $ex = $PSItem
    $auditLogMessage = "Could not disable Ortec-WS account: [$($actionContext.References.Account)]. Error: $($_.Exception.Message). Action initiated by: [$($actionContext.Origin)]"
