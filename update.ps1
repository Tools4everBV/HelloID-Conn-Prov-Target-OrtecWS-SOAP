#################################################
# HelloID-Conn-Prov-Target-OrtecWS-SOAP-Update
# PowerShell V2
#################################################
...
try {
    ...
    $correlatedAccount = $helloID_GetUserResponse.users.user | Select-Object -Property userName
    $outputContext.PreviousData = $correlatedAccount

    if ($null -ne $correlatedAccount) {
        if ($correlatedAccount.userName -ne $actionContext.Data.userName) {
    ...
    switch ($lifecycleProcess) {
        'UpdateAccount' {
            Write-Information 'userName change detected, updating Ortec-WS account'
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Updating Ortec-WS account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-OrtecSoapRequest -CommandName 'HelloID_UpdateUser' -Body $splatHelloID_UpdateUserXmlBody
            }
            else {
                Write-Information "[DryRun] Update Ortec-WS account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }

            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = "Update account was successful, Account property(s) updated: userName]"
                    IsError = $false
                })
            break
        }
        ...
        'NoChanges' {
            Write-Information "No changes to Ortec-WS account with accountReference: [$($actionContext.References.Account)]"
            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = "Skipped updating Ortec-WS account with AccountReference: [$($actionContext.References.Account)]. Reason: No changes."
                    IsError = $false
                })
            break
        }
    ...
    $outputContext.Success = $false
    $ex = $PSItem
    $auditLogMessage = "Could not update Ortec-WS account: [$($actionContext.References.Account)]. Error: $($ex.Exception.Message)"
