######################################################################
# HelloID-Conn-Prov-Target-OrtecWS-SOAP-RevokePermission-Authorization
# PowerShell V2
######################################################################
...
try {
    ...
    switch ($lifecycleProcess) {
        'RevokePermission' {
            Write-Information 'Creating HelloID_UpdateUserAuthorization Xml body'
            $splatHelloID_UpdateUserAuthorizationXmlBody = New-OrtecSoapXMLBody @{
                psk  = $actionContext.Configuration.Psk
                User = @{
                    employeeNumber = $actionContext.References.Account
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Revoking Ortec-WS permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)]"
                $null = Invoke-OrtecSoapRequest -CommandName 'HelloID_UpdateUserAuthorization' -Body $splatHelloID_UpdateUserAuthorizationXmlBody
            }
            else {
                Write-Information "[DryRun] Revoke Ortec-WS permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)], will be executed during enforcement"
            }
        ...
        'NotFound' {
            ...
            $outputContext.Success = $true
            $outputContext.AuditLogs.Add([PSCustomObject]@{
                    Message = "Ortec-WS account: [$($actionContext.References.Account)] could not be found, indicating that it may have been deleted. Action initiated by: [$($actionContext.Origin)]"
                    IsError = $false
                })
            break
        }
    ...
    $ex = $PSItem
    $auditLogMessage = "Could not revoke Ortec-WS permission for account: [$($actionContext.References.Account)]. Error: $($_.Exception.Message). Action initiated by: [$($actionContext.Origin)]"
